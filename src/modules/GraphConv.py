...
class GraphConv(nn.Module):
    ...
    def forward(self, ego_embed, edge_index, edge_type, relation_weight, dropout):
        if dropout:
        ...
        for i in range(len(self.aggregator_layers)):
            ego_embed = self.aggregator_layers[i](ego_embed, edge_index, edge_type, relation_weight)
            if dropout:
